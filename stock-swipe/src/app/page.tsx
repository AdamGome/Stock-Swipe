...
function saveCachedStock(symbol: string, stockToSave?: Stock) {
  try {
    if (
      !stockToSave ||
      stockToSave.price <= 0 ||
      stockToSave.dataSource === "fallback"
    ) {
      return;
    }
...
function getChartValues(stock: Stock, range: ChartRange) {
  return (
    stock.chartData?.[range] || createSampleChartData(stock.price, stock.change)[range]
  );
}
...
function sortStockList(list: Stock[], sortBy: ListSort) {
  ...
  return copiedList.sort((a, b) => {
    ...
    if (sortBy === "highestPE") {
      return (parseNumberValue(b.peRatio) ?? -1) - (parseNumberValue(a.peRatio) ?? -1);
    }
    if (sortBy === "lowestPE") {
      return (
        (parseNumberValue(a.peRatio) ?? Number.MAX_SAFE_INTEGER) -
        (parseNumberValue(b.peRatio) ?? Number.MAX_SAFE_INTEGER)
      );
    }
    if (sortBy === "highestBeta") {
      return (parseNumberValue(b.beta) ?? -1) - (parseNumberValue(a.beta) ?? -1);
    }
...
function mapUniverseToStockIdea(
  stock: UniverseApiStock | RecommendationItem
): StockIdea | null {
  ...
  if (
    tags.some((tag) =>
      ["ai", "chip", "chips", "semiconductor", "semiconductors"].includes(tag)
    )
  ) {
    filters.push("ai");
  }
  if (
    ...
    tags.some((tag) => ["bank", "banks", "fintech"].includes(tag))
  ) {
    filters.push("banks");
  }
  if (tags.some((tag) => ["crypto", "bitcoin", "blockchain"].includes(tag))) {
    filters.push("crypto");
  }
  ...
  if (
    riskBucket === "high" ||
    ["small", "micro"].includes(marketCapBucket) ||
    tags.some((tag) => ["speculative", "growth", "small-cap"].includes(tag))
  ) {
    filters.push("speculative");
  }
  return {
    symbol: stock.symbol.trim().toUpperCase(),
    filters: Array.from(new Set(filters.length > 0 ? filters : ["all"])),
  };
}
...
function getCompanyInsights(stock: Stock) {
  ...
  return (
    customInsights[stock.ticker.toUpperCase()] || {
      whatItDoes:
        stock.breakdown ||
        stock.summary ||
      ...
      <div className="bg-[#0f1320] border border-slate-700 rounded-3xl w-full max-w-md p-5 shadow-2xl">
        ...
        <div className="space-y-4 mt-6">
          <div className="bg-slate-900 rounded-2xl p-4">
            <p className="text-xs text-blue-400 font-bold mb-1">What it means</p>
            <p className="text-sm text-slate-300 leading-relaxed">{help.means}</p>
          </div>
          <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-4">
            <p className="text-xs text-green-400 font-bold mb-1">Generally good</p>
            <p className="text-sm text-slate-300 leading-relaxed">{help.good}</p>
          </div>
          <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-4">
            <p className="text-xs text-red-400 font-bold mb-1">Be careful</p>
            <p className="text-sm text-slate-300 leading-relaxed">{help.careful}</p>
          </div>
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-2xl p-4">
            <p className="text-xs text-yellow-300 font-bold mb-1">Beginner tip</p>
            <p className="text-sm text-slate-300 leading-relaxed">{help.tip}</p>
          </div>
        </div>
        <button
          onClick={onClose}
...
function OptionalDataCard(props: {
  label: string;
  value?: string;
  note?: string;
  compact?: boolean;
      ...
      <button type="button" onClick={onExpand} className="w-full text-left" disabled={!onExpand}>
        <div className="flex items-end justify-between mb-2">
          <div>
            ...
            <p className={isPositive ? "text-sm font-bold text-green-400" : "text-sm font-bold text-red-400"}>
              {formatSignedMoney(stats.change)} {formatSignedPercentValue(stats.changePercent)}
            </p>
          </div>
          {onExpand && <p className="text-xs text-green-400">Tap chart to expand</p>}
        </div>
        <svg viewBox="0 0 260 100" className={size === "large" ? "w-full h-64" : "w-full h-24"}>
          <polyline
            points={chartPoints}
            fill="none"
      ...
      <div className="bg-[#0f1320] border border-slate-700 rounded-3xl w-full max-w-3xl p-5 max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-600 mt-1">Expanded price chart</p>
          </div>
        ...
        <div className="mt-6 bg-[#090d18] border border-slate-800 rounded-3xl p-5">
          <div className="flex items-end justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-4xl font-black">${stats.end || stock.price}</p>
            </div>
            <div className="text-right">
              <p className={isPositive ? "text-2xl font-bold text-green-400" : "text-2xl font-bold text-red-400"}>
                {formatSignedMoney(stats.change)}
              </p>
              <p className={isPositive ? "text-sm text-green-400" : "text-sm text-red-400"}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
            <DataCard label="Range start date" value={formatChartDate(firstPoint?.datetime)} />
            <DataCard label="Range end date" value={formatChartDate(lastPoint?.datetime)} />
          </div>
        </div>
    ...
    async function loadNews() {
      try {
        ...
        const data = await apiGet<{
          ...
          dataSource?: string;
          warning?: string;
        }>(`/api/news?symbol=${stock.ticker}`, "Could not load news.");
        ...
        if (!ignore) {
          ...
          if (cachedNews) {
            ...
            setNews([]);
            setNewsWarning(
              error instanceof Error ? error.message : "News could not be loaded right now."
            );
          }
        }
      } finally {
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
      <div className="bg-[#0f1320] border border-slate-700 rounded-3xl max-w-md w-full p-5 max-h-[88vh] overflow-y-auto shadow-2xl">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-start gap-2">
              <div>
                ...
                <p className={`text-3xl font-black ${match.color}`}>{match.score}%</p>
              </div>
              <button type="button" onClick={() => onHelp("matchScore")} className={helpButtonClass}>
                ?
              </button>
            </div>
            <div className="text-right">
        ...
        <div className="grid grid-cols-2 gap-3 mt-5">
          <DataCard label="Price" value={`$${stock.price}`} helpId="price" onHelp={onHelp} />
          <DataCard label="Today" value={stock.change} note={stock.changeDollar || "$0.00"} helpId="change" onHelp={onHelp} />
          <DataCard label="Previous close" value={stock.previousClose || "N/A"} helpId="previousClose" onHelp={onHelp} />
          <OptionalDataCard label="Analyst target" value={stock.analystTargetPrice} helpId="analystTargetPrice" onHelp={onHelp} />
          <OptionalDataCard label="Market cap" value={stock.marketCap} helpId="marketCap" onHelp={onHelp} />
          <OptionalDataCard label="P/E" value={stock.peRatio} helpId="peRatio" onHelp={onHelp} />
          <OptionalDataCard label="EPS" value={stock.eps} helpId="eps" onHelp={onHelp} />
          <OptionalDataCard label="Beta" value={stock.beta} note="Volatility" helpId="beta" onHelp={onHelp} />
          <OptionalDataCard label="Profit margin" value={stock.profitMargin} helpId="profitMargin" onHelp={onHelp} />
          <OptionalDataCard label="Dividend yield" value={stock.dividendYield} helpId="dividendYield" onHelp={onHelp} />
        </div>
        <div className="mt-4">
        ...
        <div className="bg-slate-900 rounded-2xl p-4 mt-4">
          <p className="text-sm text-green-400 font-bold mb-2">What the company does</p>
          <p className="text-slate-200 leading-relaxed">{insights.whatItDoes}</p>
        </div>
        <div className="bg-slate-900 rounded-2xl p-4 mt-4">
          <p className="text-sm text-blue-400 font-bold mb-3">Key things to watch</p>
        ...
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-2xl p-4 mt-4">
          <p className="text-sm text-yellow-300 font-bold mb-2">Beginner verdict</p>
          <p className="text-sm text-slate-200 leading-relaxed">{insights.verdict}</p>
        </div>
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="text-right shrink-0">
          ...
          <p className={isPositive ? "text-xs text-green-400" : "text-xs text-red-400"}>
            {stock.change}
          </p>
        </div>
      </div>
      ...
      {stocks.length === 0 ? (
        ...
        <div className="space-y-3">
          {stocks.slice(0, 5).map((stock) => (
            <DashboardStockRow key={`${title}-${stock.ticker}`} stock={stock} onSelectStock={onSelectStock} />
          ))}
        </div>
      )}
    </div>
...
function getDashboardSections(stocks: Stock[]) {
  ...
  return {
    ...
    highestBeta: [...usefulBeta]
      .sort((a, b) => (parseNumberValue(b.beta) ?? 0) - (parseNumberValue(a.beta) ?? 0))
      .slice(0, 5),
    dividendStocks: [...dividendStocks]
      .sort(
        (a, b) =>
          (parseNumberValue(b.dividendYield) ?? 0) -
          (parseNumberValue(a.dividendYield) ?? 0)
      )
      .slice(0, 5),
    nearHigh: [...nearHigh]
      .sort((a, b) => (getWeekRangePosition(b) ?? 0) - (getWeekRangePosition(a) ?? 0))
      .slice(0, 5),
    nearLow: [...nearLow]
      .sort((a, b) => (getWeekRangePosition(a) ?? 100) - (getWeekRangePosition(b) ?? 100))
      .slice(0, 5),
  };
}
...
function RecommendationPanel({
  ...
  if (isLoading) {
    return (
      <div className="mt-4 bg-[#0f1320] border border-slate-800 rounded-2xl p-4 text-sm text-slate-400">
  ...
  return (
    <div className="mt-4 bg-[#0f1320] border border-slate-800 rounded-3xl p-4">
      ...
      {warning && <p className="text-xs text-yellow-400 mt-3">{warning}</p>}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-4">
        {recommendations.slice(0, 6).map((item) => {
          const score = item.recommendationScore ?? item.recommendation_score ?? 0;
          return (
            <button
              key={item.symbol}
              type="button"
              onClick={() => onAddTicker(item.symbol)}
              className="text-left bg-[#090d18] border border-slate-800 rounded-2xl p-3 hover:border-purple-400 transition"
            >
              <div className="flex justify-between gap-3">
              ...
              <p className="text-xs text-slate-500 truncate mt-1">
                {item.name || item.industry || item.sector || "Niche stock idea"}
              </p>
              {(item.reasons || []).slice(0, 2).map((reason) => (
                <p key={reason} className="text-[11px] text-slate-400 mt-2">
                  - {reason}
                </p>
              ))}
            </button>
...
function ListsPanel({
  ...
  onAddRecommendedTicker: (symbol: string) => void;
}) {
  ...
  const selectedSortLabel =
    listSortOptions.find((option) => option.id === sortBy)?.label || "Saved order";
  const averageMatch =
    liked.length === 0
      ? 0
      : Math.round(
    ...
    <div className="fixed inset-0 bg-[#080c16] text-white z-40 p-6 overflow-y-auto">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center gap-4">
          ...
          <button onClick={onClose} className="text-slate-500 hover:text-white text-3xl">
            x
          </button>
        </div>
          ...
          <div className="mt-8">
            {liked.length === 0 ? (
              <div className="border border-slate-800 bg-[#0f1320] rounded-3xl p-6 text-slate-400">
                No watchlist stocks yet. Swipe right on stocks to build your dashboard.
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  ...
                  <div className="bg-[#0f1320] border border-slate-800 rounded-2xl p-4">
                    ...
                    <p className="text-3xl font-black mt-1 text-green-400">{averageMatch}%</p>
                  </div>
                  <div className="bg-[#0f1320] border border-slate-800 rounded-2xl p-4">
                    <p className="text-xs text-slate-500">Up today</p>
                    <p className="text-3xl font-black mt-1 text-blue-400">{positiveCount}</p>
                  </div>
                  <div className="bg-[#0f1320] border border-slate-800 rounded-2xl p-4">
                    <p className="text-xs text-slate-500">High risk</p>
                    <p className="text-3xl font-black mt-1 text-red-400">{highRiskCount}</p>
                  </div>
                </div>
                <div className="mt-4 bg-[#0f1320] border border-slate-800 rounded-3xl p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
                    ...
                    <div className="bg-[#090d18] rounded-2xl p-4">
                      ...
                      <p className="text-sm text-slate-300 mt-2">
                        {highRiskCount > 0
                          ? `${highRiskCount} saved stock${highRiskCount === 1 ? " is" : "s are"} marked high risk.`
                    ...
                    <div className="bg-[#090d18] rounded-2xl p-4">
                      ...
                      <p className="text-sm text-slate-300 mt-2">
                        {dividendCount > 0
                          ? `${dividendCount} saved stock${dividendCount === 1 ? " has" : "s have"} a dividend yield.`
                ...
                <RecommendationPanel
                  ...
                  onAddTicker={onAddRecommendedTicker}
                />
            ...
            <div className="mt-6 bg-[#0f1320] border border-slate-800 rounded-2xl p-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-300 mt-1">{selectedSortLabel}</p>
                </div>
                <select
                  value={sortBy}
            ...
            <div className="mt-8">
              {list.length === 0 ? (
                ...
                <div className="space-y-4">
                  {list.map((stockItem, index) => {
                    const rowPositive = stockItem.change.startsWith("+");
                    const rowMatch = getMatchScore(stockItem);
                    return (
                      <div
                        key={`${stockItem.ticker}-${index}`}
                        className="bg-[#0f1320] border border-slate-800 rounded-2xl p-4"
                      >
                        <div className="flex items-center justify-between gap-4">
                          <div>
                            <div className="flex items-center gap-2">
                              <h3 className="font-black text-lg">{stockItem.ticker}</h3>
                              <span className="text-[10px] bg-green-500/20 text-green-300 border border-green-500/30 px-2 py-0.5 rounded-full">
                                {stockItem.dataSource || "saved"}
                              </span>
                            </div>
                            <p className="text-sm text-slate-500">{stockItem.name}</p>
                            <div className="flex flex-wrap gap-2 mt-2">
                              <span className={`text-xs ${rowMatch.color}`}>Match: {rowMatch.score}%</span>
                              {hasUsefulValue(stockItem.peRatio) && (
                                <span className="text-xs text-slate-600">P/E: {stockItem.peRatio}</span>
                              )}
                              {hasUsefulValue(stockItem.beta) && (
                                <span className="text-xs text-slate-600">Beta: {stockItem.beta}</span>
                              )}
                              <span className="text-xs text-slate-600">
                                Risk: {stockItem.riskLevel || "Research"}
                              </span>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="font-bold">${stockItem.price}</p>
                            <p className={rowPositive ? "text-sm text-green-400" : "text-sm text-red-400"}>
                              {stockItem.change}
                            </p>
                            <p className={rowPositive ? "text-xs text-green-400" : "text-xs text-red-400"}>
                              {stockItem.changeDollar || "$0.00"}
                            </p>
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-2 mt-4">
                          <button
                            onClick={() => onSelectStock(stockItem)}
                          ...
                          {activeTab === "watchlist" ? (
                            <button
                              onClick={() => onMoveToPassed(stockItem)}
                            ...
                            <button
                              onClick={() => onMoveToWatchlist(stockItem)}
                        ...
                        <button
                          onClick={() => onRemoveFromList(stockItem, activeTab)}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function loadUniverse() {
      try {
        const data = await apiGet<{ stocks?: UniverseApiStock[] }>(
          "/api/universe",
  ...
  useEffect(() => {
    ...
    async function loadRecommendations() {
      ...
      try {
        ...
        setRecommendationWarning("");
        const seenSymbols = [...liked, ...passed].map((savedStock) =>
        ...
        if (!ignore) {
          setRecommendations([]);
          setRecommendationWarning(
            error instanceof Error ? error.message : "Recommendations could not be loaded right now."
          );
        }
      } finally {
  ...
  const recommendedSymbols = useMemo(
    () => recommendations.map((item) => item.symbol.toUpperCase()),
    [recommendations]
  );
  ...
  const allStockSymbols = useMemo(() => {
    ...
    const orderedSymbols = [
      ...
      ...filteredDefaultSymbols,
      ...defaultStockSymbols,
    ]
      .map((symbol) => symbol.toUpperCase())
      .filter(Boolean);

    return orderedSymbols.filter(
      (symbol, index) => orderedSymbols.indexOf(symbol) === index
    );
  }, [extraSymbols, recommendedSymbols, filteredDefaultSymbols]);

  const alreadySeenTickers = useMemo(
    () => [...liked, ...passed].map((savedStock) => savedStock.ticker.toUpperCase()),
    [liked, passed]
  );
  ...
  const blockedTickers = useMemo(
    () => [
      ...alreadySeenTickers,
      ...unavailableSymbols.map((symbol) => symbol.toUpperCase()),
    ],
    [alreadySeenTickers, unavailableSymbols]
  );

  const currentSymbol = allStockSymbols.find(
    (symbol) => !blockedTickers.includes(symbol.toUpperCase())
  );

  const totalSaved = liked.length + passed.length;
  const selectedFilterLabel =
    filterOptions.find((filter) => filter.id === selectedFilter)?.label || "All";
      ...
      try {
        ...
        const [stockData, chartData] = await Promise.all([
          ...
          apiGet<{
            chartData?: Partial<Record<ChartRange, number[]>>;
            chartPoints?: Partial<Record<ChartRange, ChartPoint[]>>;
            stats?: Partial<Record<ChartRange, ChartStats>>;
          }>(`/api/chart?symbol=${symbolToLoad}`, "Could not load chart data."),
        ]);
        const incomingStock: Stock = {
          ...stockData,
          chartData:
            chartData.chartData || createSampleChartData(stockData.price, stockData.change),
          chartPoints: chartData.chartPoints,
          chartStats: chartData.stats,
        };
        const finalStock =
          stockData.dataSource === "fallback" && cachedStock ? cachedStock : incomingStock;
        ...
        if (!ignore) {
          ...
          if (cachedStock) {
            ...
            console.warn("Skipping unavailable stock:", symbolToLoad, error);
            setUnavailableSymbols((current) =>
              current.includes(symbolToLoad.toUpperCase())
                ? current
                : [...current, symbolToLoad.toUpperCase()]
            );
            setFetchError("");
            setStock(null);
          }
  ...
  function addTickerToDeck(symbolFromButton?: string) {
    const symbol = (symbolFromButton || tickerInput)
      .trim()
      .toUpperCase()
      .replace(/[^A-Z0-9.-]/g, "");
    if (!symbol) {
    ...
    setUnavailableSymbols((current) => current.filter((savedSymbol) => savedSymbol !== symbol));
    saveExtraSymbols([
      symbol,
      ...extraSymbols.filter((savedSymbol) => savedSymbol.toUpperCase() !== symbol),
    ]);
    setTickerInput("");
  ...
  function skipCurrentStock() {
    ...
    setUnavailableSymbols((current) =>
      current.includes(currentSymbol.toUpperCase())
        ? current
        : [...current, currentSymbol.toUpperCase()]
    );
    setFetchError("");
    setStock(null);
    setSelectedRange("1M");
  ...
  function retryCurrentStock() {
    ...
    setUnavailableSymbols((current) =>
      current.filter((symbol) => symbol !== symbolToRetry.toUpperCase())
    );
    Promise.all([
      ...
      apiGet<{
        ...
        chartPoints?: Partial<Record<ChartRange, ChartPoint[]>>;
        stats?: Partial<Record<ChartRange, ChartStats>>;
      }>(`/api/chart?symbol=${symbolToRetry}`, "Could not load chart data."),
    ])
      ...
      .catch((error) => {
        ...
        console.warn("Retry failed, skipping unavailable stock:", symbolToRetry, error);
        setUnavailableSymbols((current) =>
          current.includes(symbolToRetry.toUpperCase())
            ? current
            : [...current, symbolToRetry.toUpperCase()]
        );
        setFetchError("");
        setStock(null);
      });
      ...
      <header className="h-16 border-b border-slate-800 flex items-center justify-between px-5 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 mt-1">{selectedFilterLabel} deck</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
    ...
    <div className="fixed inset-0 bg-black/80 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-[#0f1320] border border-slate-700 rounded-3xl w-full max-w-md p-5 shadow-2xl max-h-[88vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowFilters(false)} className="text-slate-400 hover:text-white text-3xl">
            x
          </button>
        </div>
        <div className="mt-6">
        ...
        <div className="mt-6 border-t border-slate-800 pt-5">
          ...
          >
            ...
            />
            <button type="submit" className="bg-green-400 text-black px-5 py-3 rounded-full font-bold">
              Add
            </button>
          </form>
          ...
          {tickerMessage && <p className="text-xs text-slate-500 mt-3">{tickerMessage}</p>}
        </div>
        <div className="mt-6 bg-[#090d18] border border-slate-800 rounded-2xl p-4">
          <p className="text-sm text-slate-300 font-bold">Shared cache is on</p>
          <p className="text-xs text-slate-500 mt-2 leading-relaxed">
            Stock and chart data can be saved in Supabase so everyone can reuse recent data without spending extra API calls.
          </p>
        </div>
      </div>
    </div>
    ...
    <>
      ...
      {showLists && (
        <ListsPanel
          ...
          onAddRecommendedTicker={(symbol) => addTickerToDeck(symbol)}
        />
      )}
      {selectedStock && (
        <StockDetailModal
      ...
      <main className="min-h-screen bg-[#080c16] text-white overflow-x-hidden">
        ...
        <section className="min-h-[calc(100vh-64px)] flex flex-col items-center justify-center p-6 text-center">
          <div className="max-w-sm w-full bg-[#0f1320] border border-slate-800 rounded-3xl p-6">
            <h2 className="text-4xl font-bold">No more available stocks</h2>
            <p className="text-slate-500 mt-4">
              You have swiped through the available stocks in the {selectedFilterLabel} deck. Some tickers were skipped because live data could not be loaded.
            </p>
            {lastAction && (
              <button
            ...
            <div className="flex flex-col gap-3 mt-8">
              ...
              <button
                onClick={() => setUnavailableSymbols([])}
                className="border border-slate-700 text-slate-300 px-6 py-3 rounded-full"
              >
                Retry skipped tickers
              </button>
              <button
                onClick={resetApp}
      ...
      <main className="min-h-screen bg-[#080c16] text-white overflow-x-hidden">
        ...
        <section className="min-h-[calc(100vh-64px)] flex items-center justify-center p-6">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold">Finding available stock data...</h2>
            <p className="text-slate-500 mt-2">
              Checking {currentSymbol} from {selectedFilterLabel}
            </p>
          </div>
        </section>
        {modals}
      ...
      <main className="min-h-screen bg-[#080c16] text-white overflow-x-hidden">
        ...
        <section className="min-h-[calc(100vh-64px)] flex items-center justify-center p-6">
          <div className="bg-[#0f1320] border border-slate-800 rounded-3xl p-6 max-w-md w-full text-center">
            <h2 className="text-2xl font-bold">Could not load {currentSymbol}</h2>
            ...
            <button onClick={skipCurrentStock} className="mt-4 block mx-auto text-slate-500 underline">
              Skip this stock
            </button>
          </div>
        </section>
        ...
        >
          ...
          <div className={`mb-4 rounded-2xl border ${match.bg} ${match.border} p-3`}>
            <div className="flex justify-between items-center">
              ...
              <p className={`text-sm font-bold ${match.color}`}>{match.label}</p>
            </div>
          </div>
          <h2 className="text-5xl font-black">{stock.ticker}</h2>
          <p className="text-lg mt-2 text-slate-300 line-clamp-1">{stock.name}</p>
          <div className="flex justify-center items-end gap-4 mt-5">
            <p className="text-3xl font-bold">${stock.price}</p>
            <div className="text-left">
              <p className={isPositive ? "text-green-400" : "text-red-400"}>{stock.change}</p>
              <p className={isPositive ? "text-xs text-green-400" : "text-xs text-red-400"}>
          ...
          <div className="grid grid-cols-3 gap-2 mt-2 text-left">
            <OptionalDataCard
              ...
              onHelp={setActiveMetricHelp}
            />
            <OptionalDataCard compact label="P/E" value={stock.peRatio} helpId="peRatio" onHelp={setActiveMetricHelp} />
            <DataCard compact label="Risk" value={stock.riskLevel || "Research"} helpId="riskLevel" onHelp={setActiveMetricHelp} />
          </div>
          <div className="mt-4">
            <WeekRangeBar stock={stock} compact onHelp={setActiveMetricHelp} />
          </div>
          <button
